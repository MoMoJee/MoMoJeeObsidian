...
这和[[4.2.4 两平行平面波的叠加与干涉条纹空间频率|平面波叠加中的相位差]]是同一个思想：空间位置差通过光程差变成相位差，相位差再决定相干叠加结果。
...
本节的位移-相移定理就是推导多缝干涉因子的准备。它把“结构在空间中的周期排列”转化成“远场中的相位等差数列”。这也和[[4.2.4 两平行平面波的叠加与干涉条纹空间频率#三、 条纹间距与==空间频率==|空间频率]]有关：空间周期越小，相邻单元的相位随角度变化越快，远场图样中的细节也越密。
